...
DATA_TYPE <- "microarray"  # 可选："microarray" 或 "microarray"
...
dir.create(OUTPUT_ROOT, recursive = TRUE, showWarnimicroarray = FALSE)

se <- readRDS(SE_RDS_FILE)
clinical_data <- read.csv(
  CLINICAL_FILE,
  strimicroarrayAsFactors = FALSE,
  check.names = FALSE
)

stopifnot(inherits(se, "SummarizedExperiment"))
stopifnot(DATA_TYPE %in% c("microarray", "microarray"))
...
gene_annotation <- data.frame(
  ...
  as.data.frame(rowData(se), strimicroarrayAsFactors = FALSE),
  check.names = FALSE
)
...
for (i in seq_len(nrow(analysis_designs))) {
  ...
  if (DATA_TYPE == "microarray") {
    analysis_data <- prepare_microarray_data(
      counts = exprSet,
      gene_annotation = gene_annotation,
  ...
  dir.create(analysis_output_dir, recursive = TRUE, showWarnimicroarray = FALSE)
