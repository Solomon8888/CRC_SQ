...
# ANALYSES_TO_PLOT <- c("LD_ALL", "MD_ALL", "HD_ALL")
...
# 行列聚类设置。行按Up/Down分块但不显示文字标题；列按实验组/对照组分块，组内按真实表达模式聚类。
CLUSTER_ROWS <- FALSE
...
# 样本分组颜色。每个分析的实验组使用红色；对照组使用深灰。
CONTROL_GROUP_COLOR <- "#6A6A6A"
EXPERIMENT_GROUP_COLOR <- "#D73027"
DISPLAY_CONTROL_GROUP <- "Control"
DISPLAY_EXPERIMENT_GROUP <- "Experiment"
DIRECTION_COLORS <- c(
  Up = "#D73027",
  Down = "#2166AC"
)
