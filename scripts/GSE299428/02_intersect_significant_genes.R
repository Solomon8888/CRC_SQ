...
# 交集结果统一保存到：
# results/ngs/GSE299428/intersect/<intersection_scheme>/
#
...
#   oxaliplatin_high_common = c("HD_ALL", "HD_HCT116", "HD_SW480")
# )
INTERSECTION_SCHEMES <- "auto_all"

# 运行哪些交集方案。默认"all"表示运行INTERSECTION_SCHEMES中的全部方案。
SCHEMES_TO_RUN <- "all"
# SCHEMES_TO_RUN <- c("oxaliplatin_high_common")
...
# 交集后的基因在单个差异分析结果中展示。
# GSE299428的analysis名称后续可能包含下划线；默认不按下划线拆分分析名。
OUTPUT_SINGLE_ANALYSIS_ONLY <- FALSE
...
if (identical(INTERSECTION_SCHEMES, "auto_all")) {
  if (length(file_info$Analysis_Name) < 2) {
    stop("At least two DEG analyses are required to build the automatic intersection scheme.")
  }
