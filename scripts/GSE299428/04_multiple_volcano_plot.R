...
DE_SCRIPT_FILE <- "scripts/GSE299428/01_limma_differential_expression.R"
...
# 自动构建ALL_ANALYSES方案。等你完成clinical_edit.csv分组设计后，
# 可以改成手动list，例如：
# MULTIPLE_VOLCANO_SCHEMES <- list(
#   oxaliplatin_all_doses = c("LD_ALL", "MD_ALL", "HD_ALL")
# )
MULTIPLE_VOLCANO_SCHEMES <- "auto_all"

# 运行哪些多组火山图方案。默认"all"表示运行MULTIPLE_VOLCANO_SCHEMES中的全部方案。
SCHEMES_TO_RUN <- "all"
# SCHEMES_TO_RUN <- c("oxaliplatin_all_doses")
...
# CUSTOM_LABEL_GENES <- list(all = c("ATF3"), HD_ALL = c("ABCB1"))
CUSTOM_LABEL_GENES <- character(0)

# 多组火山图中Top基因上下避让距离。其他Top基因标注风格来自公共配置文件。
TOP_GENE_LABEL_NUDGE_Y <- 0.80

# 每组中间的彩色标签；未列出的分析会自动分配颜色。
GROUP_LABEL_COLORS <- character(0)
...
if (identical(MULTIPLE_VOLCANO_SCHEMES, "auto_all")) {
  if (length(file_info$Analysis_Name) < 2) {
    stop("At least two DEG analyses are required to build the automatic multiple-volcano scheme.")
  }

  MULTIPLE_VOLCANO_SCHEMES <- list(
    ALL_ANALYSES = file_info$Analysis_Name
  )
}
